    ...
    <section className="max-w-5xl mx-auto py-10 px-3 sm:px-6">
      ...
      {items.length === 0 ? (
        <div className="border border-dashed border-slate-300 rounded-xl p-8 text-center bg-slate-50">
          ...
          <Link
            to="/"
            className="inline-flex mt-3 px-5 py-2 rounded-lg bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 transition"
          >
            Перейти к каталогу
          </Link>
        </div>
        ...
        <div className="grid gap-5">
          <div className="grid gap-4">
            {items.map((item) => (
              <article
                key={item.product.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200 rounded-xl p-4 shadow-sm"
              >
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg border border-red-500 text-red-600 text-sm font-medium hover:bg-red-50 transition"
                  >
                    Удалить из корзины
                  </button>
                </div>
